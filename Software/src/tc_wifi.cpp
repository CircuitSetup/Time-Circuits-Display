...
WiFiManagerParameter custom_autoRotateTimes("rotate_times", "<br>Time-rotation interval<br>(0=off, 1-5=every 5/10/15/30/60th minute)",settings.autoRotateTimes, 1, "type='number' min='0' max='5' autocomplete='off' title='Selects the interval for automatic time-cycling when idle'");
...
WiFiManagerParameter custom_speedoFact("speFac", "Speedo sequence speed factor (0.5-5.0)", settings.speedoFact, 3, "type='number' min='0.5' max='5.0' step='0.5' title='1.0 means the sequence is played in real-world DeLorean acceleration time. Higher values make the sequence run faster, lower values slower'");
#endif

WiFiManagerParameter custom_copyAudio("cpAu", "Audio file installation: Write COPY here to copy the original audio files from the SD card to the internal flash file system", settings.copyAudio, 4, "autocomplete='off'");
...
void updateConfigPortalValues()
{
    ...
    custom_autoRotateTimes.setValue(settings.autoRotateTimes, 1);
    ...
    custom_copyAudio.setValue("", 4);   // Always clear
}
